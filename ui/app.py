...
def render_styled_pool(col, title, pool_id, data_list, cols, empty_msg="未扫到任何标的。"):
    with col:
        ...
        try:
            ...
            # 【V26.6 性能优化】单次 apply，涨跌/仓位用字典预查替代字符串解析
            # 行样式：只对「代码」「名称」列应用战法颜色，避免整行遍历
            _key_cols = {'代码', '名称'}

            def _make_row_styler(row):
                strategy = str(row.get('战法', ''))
                name_cell = str(row.get('名称', ''))
                if (
                    '【缩量期备选】' in strategy or '【缩量期备选】' in name_cell
                    or name_cell.strip().startswith('🔭')
                    or '高风险备选' in strategy or '仅供观察' in strategy
                ):
                    color = 'background-color: #fff7ed; color: #c2410c; font-weight: 600;'
                elif '短共振' in strategy or '短狙击' in strategy:
                    color = 'background-color: #8E24AA; color: #FFA726; font-weight: bold;'
                elif '月共振' in strategy:
                    color = 'background-color: #01579B; color: #FFFFFF; font-weight: bold;'
                else:
                    color = ''
                if color:
                    return {c: color for c in _key_cols}
                return {}

            # 涨幅颜色：按阈值直接映射，零字符串解析开销
            _pct_color = {}
            if '涨幅' in df.columns:
                for val in df['涨幅']:
                    try:
                        n = float(str(val).replace('%', '').strip())
                        _pct_color[val] = 'color: #ef4444; font-weight: bold;' if n > 0 else ('color: #10b981; font-weight: bold;' if n < 0 else '')
                    except (TypeError, ValueError):
                        _pct_color[val] = ''
            ...
            if '建议仓位' in df.columns:
                for val in df['建议仓位']:
                    s = str(val)
                    if '🔥' in s or '主攻' in s:
                        _pos_color[val] = 'color: #ef4444; font-weight: bold;'
                    elif '🛑' in s or '极危' in s:
                        _pos_color[val] = 'color: #f97316; font-weight: bold;'
                    elif ('避险' in s or '重装' in s or '压舱' in s):
                        _pos_color[val] = 'color: #3b82f6; font-weight: bold;'
                    elif ('均衡' in s or '游击' in s or '常规' in s):
                        _pos_color[val] = 'color: #8b5cf6; font-weight: bold;'
                    else:
                        _pos_color[val] = ''

            def _apply_row_styles(row):
                styles = {c: '' for c in df.columns}
                row_style = _make_row_styler(row)
                styles.update(row_style)
                pct_c = _pct_color.get(row.get('涨幅'), '')
                if pct_c:
                    styles['涨幅'] = pct_c
                pos_c = _pos_color.get(row.get('建议仓位'), '')
                if pos_c:
                    styles['建议仓位'] = pos_c
                return [styles.get(c, '') for c in df.columns]
            ...
            styled_df = (
                df.style.set_table_styles(_base_style)
                .format(format_dict)
                .apply(_apply_row_styles, axis=1)
            )
